            ...
            <form method="POST" action="{{ route('cabinet.store') }}">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" rows="5" required 
                              style="width: 100%; padding: 10px; margin: 5px 0; border: 1px solid #ddd; border-radius: 4px;">{{ old('description') }}</textarea>
                    @error('description')
                        <span style="color: red; font-size: 12px;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="max_participants" value="{{ old('max_participants', 10) }}" 
                ...
                <div class="form-group">
                    <button type="submit" class="btn" id="submit-btn"
                            style="background: #4CAF50; color: white; padding: 10px 30px; border: none; cursor: pointer; border-radius: 4px;">
                        Отправить
                    </button>
                </div>
        ...
        try {
            ...
            for (let i = 0; i < options.length; i++) {
                ...
                if (data.occupied.includes(option.value)) {
                    ...
                    // Восстанавливаем текст если был изменен
